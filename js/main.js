...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        initializeSkills(config.skills);
        initializeCertifications(config.certifications);
        initializeAchievements(config.achievements);
        initializeContact(config.contact);

        // Initialize projects
        await initializeProjects(config.featuredProjects);
    } catch (error) {
...
function getDefaultConfig() {
    return {
        profile: {
            name: 'Vikas Rathi',
            role: 'Data Engineer',
            tagline: 'Building data pipelines and AI solutions',
            linkedin: '#'
        },
...
function initializeProfile(config) {
    ...
    if (roleEl && profile.role) {
        roleEl.textContent = profile.company
...
function initializeCertifications(certifications) {
    ...
    if (!container || !certifications || certifications.length === 0) {
    ...
    container.innerHTML = certifications.map((cert, index) => `
        <div class="cert-card" data-aos="fade-up" data-aos-delay="${index * 100}">
            ...
            ${cert.year ? `<p class="cert-year">${escapeHtml(cert.year)}</p>` : ''}
        </div>
    `).join('');
}
...
function initializeAchievements(achievements) {
    ...
    container.innerHTML = achievements.map((achievement, index) => `
        <div class="achievement-item" data-aos="fade-up" data-aos-delay="${index * 100}">
            ...
            ${achievement.description ? `<p class="achievement-description">${escapeHtml(achievement.description)}</p>` : ''}
        </div>
    `).join('');
}
...
// Expose filter initialization for github.js
